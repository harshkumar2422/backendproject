    ...
    if (!user) {
      ...
      if (vatUsed == "true") {
        if (vatNumber != (null || undefined)) {
          ...
          return res
            .status(404)
            .send({ message: "Vat number Not provided", data: [] });
        }
      } else {
        if (vatNumber != (null || undefined)) {
          return res
            .status(404)
            .send({ message: "Then first chose yes", data: [] });
        }
      }
...
exports.signin = async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      $or: [{ email: userEmail }, { email: email }],
      userType: "USER",
    });
    if (!user) {
      return res
      ...
      if (user.status == "Pending") {
        return res
          ...
          .send({
            status: 201,
            message: "User verification is pending login after some time.",
          });
      } else {
        ...
        if (!isValidPassword) {
          return res
            .status(401)
            .send({ status: 401, message: "Wrong password" });
        }
        const accessToken = jwt.sign({ id: user._id }, authConfig.secret, {
          expiresIn: authConfig.accessTokenTime,
        });
        let findAddress = await userAddress.findOne({
          userId: user._id,
          type: "Registration",
        });
        return res
    ...
    if (!data) {
      ...
      if (info) {
        ...
        res
          .status(200)
          .json({
            message: "Otp not send on your mail please check.",
            status: 200,
            data: {},
          });
      }
    ...
    if (phone != (null || undefined)) {
      ...
      if (findAddress) {
        await userAddress.findByIdAndUpdate(
          { _id: findAddress._id },
          { $set: { phone: phone || findAddress.phone } },
          { new: true }
        );
      }
    } else {
...
exports.addAdress = async (req, res) => {
  try {
    const { address, addressComplement, city, pincode, country, phone } =
      req.body;
    const user = await User.findById(req.user._id);
    if (!user) {
      return res.status(404).send({ message: "not found" });
  ...
  try {
    ...
    if (!user) {
      ...
      if (findData.type == "Registration") {
        return res
          .status(201)
          .send({ message: "Registration address can not update.", data: {} });
      } else {
        let obj = {
          userId: user._id,
    ...
    if (!user) {
      ...
      if (!findData) {
        ...
        if (findData.type == "Registration") {
          return res
            .status(201)
            .send({ message: "Registration address not delete.", data: {} });
        } else {
                ...
                if (findProduct.colorActive == true) {
                    ...
                    if (findColor.size == true) {
                      if (findColor.colorSize.length > 0) {
                          ...
                          if (
                            ...
                            if (findProduct.taxInclude == true) {
                              ...
                              productTotalTax = Number(
                                (price * req.body.quantity * tax) / 100
                              ).toFixed(2);
                            } else {
                              tax = tax;
                            }
                            let productPaid =
                              Number((price * req.body.quantity).toFixed(2)) +
                              Number(productTotalTax);
                            let obj = {
                              ...
                              total: Number(
                                (price * req.body.quantity).toFixed(2)
                              ),
                              paidAmount: productPaid,
                            };
                            totalTax =
                              Number(findCart.tax) + Number(productTotalTax);
                        ...
                        return res
                          ...
                          .send({
                            status: 409,
                            message: "Currently no size available.",
                          });
                      }
                    } else {
                      console.log(
                  ...
                  if (update) {
                    ...
                    for (let j = 0; j < update.products.length; j++) {
                      totalAmount =
                        Number(totalAmount) + Number(update.products[j].total);
                      totalTax1 =
                        Number(totalTax1) + Number(update.products[j].totalTax);
                      paidAmount =
                        Number(paidAmount) +
                        Number(update.products[j].paidAmount);
                      discount =
                        Number(discount) + Number(update.products[j].discount);
                    }
                    if (paidAmount > 250) {
                      delivery = "0";
                  ...
                  for (let i = 0; i < findColor.colorSize.length; i++) {
                    if (
                      ...
                      if (findProduct.discount == true) {
                        price = findProduct.discountPrice;
                        discount =
                          Number(findProduct.price).toFixed(2) -
                          Number(findProduct.discountPrice).toFixed(2);
                      } else {
                        price = findProduct.price;
                      }
                      let products = [],
  ...
  try {
    ...
    if (!user) {
      ...
      if (findCart) {
        ...
        return res
          .status(200)
          .send({ status: 200, message: "Cart detail not found.", data: {} });
      }
    }
  } catch (error) {
    console.log(error);
        ...
        for (var i = 0; i < findCart.products.length; i++) {
          if (findCart.products[i]._id.toString() === req.body.products_id) {
            ...
            let findProduct = await Product.findById({
              _id: findCart.products[i].productId.toString(),
            });
            if (findProduct.taxInclude == true) {
              tax = findProduct.tax;
              totalTax = Number(
                (findProduct.price * req.body.quantity * Number(tax)) / 100
              ).toFixed(2);
              total = Number(
            ...
            if (findProduct.taxInclude == true) {
              tax = findProduct.tax;
              totalTax = Number(
                (findProduct.price * req.body.quantity * tax) / 100
              ).toFixed(2);
              total = Number(
                (findProduct.price * req.body.quantity).toFixed(2)
              );
            ...
            let update1 = await Cart.findByIdAndUpdate(
              { _id: update._id },
              {
                $set: {
                  ...
                  paidAmount: Number(paidAmount2).toFixed(2),
                  tax: Number(totalTax).toFixed(2),
                  totalItem: update.products.length,
                },
              },
  ...
  try {
    ...
    if (findUserOrder) {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: `https://vaping-ecommerce-web.vercel.app/order-success/${findUserOrder.orderId}`,
        cancel_url: `https://vaping-ecommerce-web.vercel.app/order-failure/${findUserOrder.orderId}`,
  ...
  try {
    ...
    if (findUserOrder) {
      ...
      let table1 = [
        ...
        [
          `${findUserOrder.address}${findUserOrder.city}`,
          "",
          "",
          "",
          "",
        ...
        [
          `${findUserOrder.country} ${findUserOrder.pincode}`,
          "",
          "",
          "",
          "",
      ...
      let table13 = [
        ...
        [
          ...
          "",
          "",
          `${findUserOrder?.delivery}`,
          "",
          `£${findUserOrder?.paidAmount}`,
        ],
      ];
      const tableArray4 = {
        headers: ["", "", "", "", "", "", "", ""],
      ...
      doc
        .text(
          "VAT NO: GB350971689    CO RegNo:1139394    AWRS NO: XVAW00000113046",
          115,
          690
        )
        .font("Helvetica")
        .fontSize(16);
      doc
        .text("THANK YOU FOR YOUR VALUE CUSTOM", 100, 710)
        .font("Helvetica")
        .fontSize(8);
      doc
        .text(
      ...
      if (info) {
        ...
        if (info1) {
          ...
          if (deleteCart) {
            res
              .status(200)
              .json({ message: "Payment success.", status: 200, data: {} });
          }
        }
      } else {
        let deleteCart = await Cart.findOneAndDelete({
  ...
  try {
    ...
    if (findUserOrder) {
      ...
      let line_items = [],
        ...
        paidAmount = findUserOrderpaidAmount,
...
exports.successOrder3 = async (req, res) => {
  try {
    ...
    if (findUserOrder) {
      ...
      let line_items = [],
        total = findUserOrder.paidAmount,
        paidAmount = findUserOrder.paidAmount,
        tax = findUserOrder.tax,
        TotalQua = 0,
        address = findUserOrder.address,
        pincode = findUserOrder?.pincode,
